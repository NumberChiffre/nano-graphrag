...
async def nano_entity_extraction(text):
    ...
    current_result = await extract_entities(chunks, graph_storage, None, graph_storage.global_config)

    print("Current Implementation Result:")
    print("Entities:")
    entities = []
    for node, data in graph_storage._graph.nodes(data=True):
        entities.append(f"{node} ({data.get('entity_type', 'Unknown')}): {data.get('description', 'No description')}")
    print(", ".join(entities))

    print("\nRelationships:")
    relationships = []
    for source, target, data in graph_storage._graph.edges(data=True):
        relationships.append(f"{source} -> {target}: {data.get('description', 'No description')}")
    print("\n".join(relationships))
